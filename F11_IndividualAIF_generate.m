...
%DCE_series = DYN_br;
DCE_LD = permute(DCE_LD,[1,2,4,3]);
[ysize, xsize, numdyn, zsize] = size(DCE_LD);
%%
SI = [];
...
if ~isempty(artery)
    for i = 1:size(artery,1)
        SI = [SI, permute(DCE_LD(artery(i,1), artery(i,2), :, artery(i,3)), [3,1,2,4])];
    end

    SI = SI';

    %% signal -> average -> concentration
    S_pre = mean(mean(SI(:,1:inj),2)); %average all slices to get high SNR
%     ratio_0 = (1- exp(-TR_ht/T10))./(1- exp(-TR_ht/T10)*cosa); 
    S_0 = S_pre *(1-cosa*exp(-TR_ht/T10))/(sina*(1-exp(-TR_ht/T10))); %signal intensity equation
    % syms x
...
%     indAIF_concentraiton = SI2C(S_norm,sina, cosa, TR_ht, T10,rel);
    indAIF_concentraiton = (-log((S_norm-sina)./(S_norm*cosa-sina))/TR_ht - 1/T10)/rel; %eq[1]
