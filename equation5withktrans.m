...
for i = 1:length(assigned_ve)
    ...
    R_toi = equation5function2withktrans(x,t,Cp,R1i,r1,R10,fw);
    R_toi = R_toi';
...
for ii = 1:size(R_toi_values,2) %for each column in R_toi_values       
        ...
        ydata = ydata';
        [x,resnorm] = lsqcurvefit(@equation5function2withktrans,x0st,t,ydata,[],[],options,Cp,R1i,r1,R10,fw);
        calculated_ktrans(ii) = x(1);
        calculated_ve(ii) = x(2);
        calculated_Ti(ii) = x(3);
        tempfit = equation5function2withktrans([x(1) x(2) x(3)],t_new,AIF_pop,R1i,r1,R10,fw);
        %tempfit = tempfit';
