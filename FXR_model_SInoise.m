...
% Create 1x66 array with random variables
%for i = 1:66
...
% Initialize arrays 
SI_values = [];
SI_3d = zeros(66,100,10);  

% Forward fitting 
for i = 1:length(assigned_ve) % i = "vertical slice" number
    x = [assigned_ktrans(i) assigned_ve(i) assigned_Ti(i)]; %a new combination for a new vertical "slice" of the cube
    parfor k = 1:100 % k = # of runs per "vertical slice"
        SI = SIequationwithnoise_FXR(x,t,Cp,R1i,r1,R10,fw,flip,TR,S0,storerandom);  
        SI = SI';
        SI_3d(:,k,i) = SI; %store signal intensity values for that combination and that run
    end
end
 

% LSQ fit
x0st = [(0.4/60) 0.35 1.0]; %initial guesses
...
fits = zeros(66,100,10); %initialize fits 3d array

for ii = 1:10 % "vertical slice" number
    parfor kk = 1:100 %run number
        ydata = SI_3d(:,kk,ii); %access ydata for comparison
        ydata = ydata';
        [x,resnorm] = lsqcurvefit('SIequationwithktrans',x0st,t_new,ydata,[],[],options,Cp,R1i,r1,R10,fw,flip,TR,S0);
        % Store calculated values
        calculated_ktrans(kk,ii) = x(1);
        calculated_ve(kk,ii) = x(2);
        calculated_Ti(kk,ii) = x(3);
        tempfit = SIequationwithktrans([x(1) x(2) x(3)],t_new,Cp,R1i,r1,R10,fw,flip,TR,S0);
        fits(:,kk,ii) = real(tempfit);
    end
end
